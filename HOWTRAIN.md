...
분류기: EfficientNet-B4 · 분할기: U-Net (EfficientNet-B4 encoder).
...
이 문서는 `/Users/pke03/OOB_7_100base` 전용입니다. `OOB_test_7_epoch100` 및 `/Users/pke03/OOB_test_7_epoch100-main`은 baseline 참고용으로만 보고, 학습/검증/수정은 이 폴더에서만 진행합니다.
...
| CT Hemorrhage | PhysioNet (`ct-ich v1.3.1`, NIfTI) | ~1.2GB | 자동 (zip) |
...
### 3-1. 분류기 (EfficientNet-B4, 2-class)

```bash
./venv/bin/python training/train_classifier.py
```

- 데이터: CT Hemorrhage + tekno21 + BHSD 통합
- 설정: 50 epoch, batch 32, lr 1e-3, image 240×240
- 체크포인트: `checkpoints/classifier/best_classifier.pth`
- 옵션: `--epochs 30 --batch_size 8 --lr 1e-4`

### 3-2. 분할기 (U-Net + EfficientNet-B4, binary mask)

```bash
./venv/bin/python training/train_segmentor.py
```

- 데이터: CT Hemorrhage 마스크 + BHSD 마스크 통합 (~2600장)
- 설정: 80 epoch, batch 24, lr 1e-4, image 320×320
...
OOB_7_100base/
  ...
  data/
    raw/                     # (gitignore: 다운로드로 받음)
      tekno21/
      ct_hemorrhage/.../ct_scans/*.nii
      ct_hemorrhage/.../masks/*.nii
      aisd/{images,masks}/
  ...
  models/
    classifier.py            # EfficientNet-B4 wrapper
    segmentor.py             # U-Net (smp)

  training/
    train_classifier.py
    train_segmentor.py
    metrics.py               # threshold sweep, Focal/Tversky losses, Dice/IoU
...
- **PhysioNet zip 다운로드 실패** → 브라우저에서 https://physionet.org/content/ct-ich/1.3.1/ 방문해 수동 다운로드 후 `data/raw/ct_hemorrhage/` 에 압축 해제
- **BHSD 다운로드 느림** → HuggingFace CDN 상태 확인, 또는 `huggingface-cli login` 으로 인증
- **MPS OOM / GPU 메모리 부족** → 먼저 `config.yaml`의 classifier `batch_size`를 32 이하로 낮추세요. `PYTORCH_MPS_HIGH_WATERMARK_RATIO=0.0`은 시스템 프리징 위험이 있어 기본 해결책으로 쓰지 않습니다.
